...
| VIP 强牌结果特效 | ✅ 已完成 | 会员同花顺/豹子在结果页获得三牌黑金环绕高亮与一次性爆炸粒子，支持减少动态效果 |
| 断线重连优化 | ✅ 已完成 | visibilitychange 自动刷新 + Socket.IO 快速重连 |
| 部署后自动化测试 | ✅ 已完成 | 密钥保护测试页，81 项用例一键运行，实时结果+失败诊断 |
...
│   ├── result-member-highlight.js # 会员同花顺/豹子结果高亮资格判定
│   ├── test-runner.js        # 部署后自动化测试运行器（81 项用例 + 实时诊断）
...
直接访问 `http://localhost:3000/#/member-highlight-preview` 可打开公开效果预览页，无需测试密钥或创建房间。页面会并列展示同花顺与豹子，可点击“重播粒子”，也可模拟“减少动态效果”的静态高亮；系统自身的 Reduced Motion 设置始终优先。预览使用固定牌面，只渲染前端视觉，不调用房间或结算接口。

真实结果页会将服务端结算牌型与房间玩家快照中的 `memberLevel` 关联：

- 仅会员最终牌型为同花顺（`handType = 4`）或豹子（`handType = 5`）时触发，庄家和闲家均支持。
- 三张结果牌外显示局部黑金环绕高亮；进入页面时播放一次高密度金色星芒、空心星粒子与扩散冲击光环，后续保持低频光晕，不遮挡结果页操作。
- 普通玩家拿到同样牌型、或会员拿到其他牌型时不显示特效。
- 特效是纯前端瞬时展示，不写入 `Room` / `RoundResult`，不新增 Socket 事件，也不改变牌型和结算规则。
- 开启系统“减少动态效果”后会关闭循环扫光和粒子运动，仅保留静态金色边框。
...
全部 81 项浏览器测试通过，Node 单元/集成测试通过（可通过部署后测试页面与 `npm test` 验证）：
...
| 静态资源 | 首页/app.js/style.css/qrcode.min.js/test-runner.js、会员结果高亮模块与 6 个透明 WebP 道具资产均返回 200 |
...
| VIP 强牌结果特效 | 会员同花顺/豹子资格判定、两类 DOM modifier、粒子层、公开预览路由、三牌结构、重播节点替换与手动/系统减少动态效果 |
