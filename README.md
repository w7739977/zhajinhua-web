...
| 发牌限制 | 单人发牌被拒(≥2人)、非庄发牌被拒、非waiting发牌被拒 |
| 选择开牌 | 选择开部分玩家、未选中玩家保留手牌、下轮留牌玩家跳过发牌 |
| 全开 | 全开(含过庄)、全开不过庄、openedPlayerIds 追踪 |
| 中途加入 | 游戏中加入标记观战、观战者不能下注、resetRound后自动参与 |
| 权限控制 | 庄家/房主resetRound、非庄被拒、庄家下注被拒、非庄开牌被拒 |
| 踢人 | 庄家踢人、非庄踢人被拒、不能踢自己 |
| 边界防护 | 空房间号、不存在房间、缺playerId、重复下注 |
| 牌组管理 | 16轮消耗后自动洗牌重新发牌 |
| 前端资源 | 首页/app.js/style.css/qrcode.min.js 静态资源 200 |
